...
public class spawnFood : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Time.fixedTime - lastTime) >= 0.5)
        {
            lastTime = Time.fixedTime;
            //Chose food item at random.
            int randomIndex = Random.Range(0, food.Length);
            //Choose a random spawn spot based on the spawn sphere.
            Vector3 randomPosition = new Vector3(Random.Range(-20 + orientation.position.x, 21 + orientation.position.x), 15, Random.Range(-10 + orientation.position.z, 10 + orientation.position.z));
            //Spawn the food item.
